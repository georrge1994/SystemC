...
void change_state(int link, int state){
    register_in = link;
    sc_start(4, SC_NS);
    assert(register_out.read() == state);
    cout << "@" << sc_time_stamp() << " KA set in "<< state <<" state with help "<< link << endl;
}
...
int sc_main(int argc, char* argv[]) {
    ...
    change_state(1, 0); // link: 1 state: r0 -> r0 
    change_state(3, 1); // link: 3 state: r0 -> r1 
    change_state(3, 0); // link: 3 state: r1 -> r0
    change_state(0, 2); // link: 0 state: r0 -> r2  
    change_state(3, 3); // link: 3 state: r2 -> r3
    change_state(3, 2); // link: 3 state: r3 -> r2
    change_state(1, 1); // link: 1 state: r2 -> r1
    
    change_state(3, 0); // link: 3 state: r1 -> r0
    change_state(0, 2); // link: 0 state: r0 -> r2 
    
    change_state(0, 0); // link: 0 state: r2 -> r0
    
    
    // null link
    change_state(2, 0); // link: 0 state: r0 -> r3
    reset = 1; // Assert the reset
    cout << "@" << sc_time_stamp() << " Asserting reset\n" << endl;
    sc_start(1, SC_NS);
